...
ECS是一种编程范式，它将数据和行为分开。Bevy将为[你存储所有数据](/guide/14.programming/14.2intro-data)并管理[你的所有单独功能](/guide/14.programming/14.3intro-code)部分。代码将在适当的时候运行，以便你的代码可以访问它需要的任何数据来完成其任务。

这使得以一种灵活且可重用的方式编写游戏逻辑（[系统](/guide/14.programming/14.5systems)）变得容易。例如，你可以实现：
...
[阅读更多关于如何表示你的数据。](/guide/14.programming/14.2intro-data)

[阅读更多关于如何表示你的功能性。](/guide/14.programming/14.3intro-code)